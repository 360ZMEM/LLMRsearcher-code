...
parser.add_argument('--train_comp_idx',type=str, default="", help='Indexes of components.')
args, unknown_args = parser.parse_known_args() # you can specify other arguments of train_td3.py
K = config.K; ep_num = args.episode_num
...
proc = []; ep = np.ones(args.K); proc_alive = np.zeros(args.K)
for i in range(K):
    ...
    args = ['python',f'{BASE_DIR}/RL_task/train_td3.py','--iter',str(args.iter),
            '--episode_num',str(ep_num)] + comp_args + unknown_args
    proc.append(subprocess.Popen(args, stdout=subprocess.PIPE)) # hide the output of train_td3.py
# START TRAINING
