...
RWI_str = RWI_str.replace('<num_spec>',num_desc).replace('<Env_description>',
            env_desc_prompt).replace('<rew_function>',reward_func).replace('<task_short_desc>',desc_short_name)
...
while True:
    ...
    for match_idx, match in enumerate(matches):
        ...
        # generate new funcs
        match_idx += 1
        with open(config.REWARD_BASE_DIR + f'reward_ITER1_REWARD{match_idx}.py', 'w') as f:
            f.write(replaced_reward_func) 
        with open(config.WEIGHT_BASE_DIR + f'weight_ITER1_REWARD{match_idx}.txt', 'w') as f:
            f.write(match) 
