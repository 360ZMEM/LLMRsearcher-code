...
from prompts.task_independent.rew_weight_searcher_stage1 import RWS1_str, RWS1_example1, RWS1_example2
...
parser.add_argument('--human_feedback', type=str, default='<Empty>', help = 'Interface for human feedback')
...
RWS1_str = RWS1_str.replace('<num_spec>',num_desc).replace('<Env description>',
           env_desc_prompt).replace('<rew_function>',reward_func).replace('<human_feedback>,args.human_feedback').replace('<task_short_desc>',desc_short_name)
...
summarized_output = []; overall_summary = ''
...
while True:
    ...
    count_begg_sugg = 0; str_begg_sugg = ''
