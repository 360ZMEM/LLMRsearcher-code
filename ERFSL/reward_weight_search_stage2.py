...
with open(config.REWARD_BASE_DIR + 'reward_code_fin.py','r') as f: # load the reward func file
    reward_func = f.read()

RWS2_str = RWS2_str.replace('<obj_description>',obj_desc_prompt).replace('<task_short_desc>',desc_short_name)
# add weights & suggestions
weight_str = ''; suggestion_str = ''
for i in range(K):
...
for i in range(len(sugg_lines)):
    if sugg_lines[i].lower().startswith('suggestion') or sugg_lines[i].lower().startswith('**suggestion'):
        suggestion_str += (sugg_lines[i].rstrip() + '\n')
# replace
RWS2_str = RWS2_str.replace('<weight_groups>', weight_str).replace('<suggestions>',suggestion_str)
...
while True:
    ...
    for match_idx, match in enumerate(matches):
        try:
            ...
            continue
        match_idx += 1
        with open(config.REWARD_BASE_DIR + f'reward_ITER{ITER+1}_REWARD{match_idx}.py', 'w') as f:
            f.write(replaced_reward_func) 
        with open(config.WEIGHT_BASE_DIR + f'weight_ITER{ITER+1}_REWARD{match_idx}.txt', 'w') as f:
            f.write(match) 
