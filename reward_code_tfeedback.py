...
while True:
    ...
    for idx, obj in enumerate(objectives):
        ...
        reward_func_str = generate_reward_function(reward_comp_weight,reward_comp_code,obj,reward_critic=True)
        # output reward function 
        output_rew_fname = config.REWARD_COMP_DIR + f'reward_ITER{ITER_GO}_COMP{idx+1}.py'
        with open(output_rew_fname,'w') as f:
            f.write(reward_func_str)
    for i in range((len(now_obj_index_list) - 1) // K + 1):
        # run_train.py
        args = ['python',f'{config.ERFSL_DIR}/run_train.py', "--train_components",
                "train_comp_idx", str(now_obj_index_list[i*K,(i+1)*K])]+ config.arguments + unknown_args
