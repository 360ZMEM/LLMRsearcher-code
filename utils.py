    ...
    for obj in objectives:
        for idx, weight_str in enumerate(weight[obj]):
            ...
            comment_str = (
                "# " + "weight "
                if weight_str == ["w"]
                else "reward(penalty) " + f"term for {obj} (term {idx+1})"
                if reward_critic == False
                else ""
            )
