...
parser.add_argument('--border_x', type=float, default=120.,help='Area x size')
parser.add_argument('--border_y', type=float, default=120.,help='Area y size')
...
parser.add_argument('--reward_no', type=int, default=1, help='Reward choice index. When `--train_component` is specified, this index refers to the index of user requirements.')
parser.add_argument('--save_model', action='store_true')
parser.add_argument('--random_env', type=int, default=4, help='The arrangement of environment (e.g. SNs and init. position of AUVs) varies if this argument is set to 0.') 
parser.add_argument('--train_components', action='store_true', help='The reward function is a single component, rather than a integral one.') 
...
def train():
    ...
    for ep in range(args.episode_num):
        ...
        if ep % args.save_model_freq == 0 and ep != 0:
            ...
            print_reward_mean = []; print_reward_std = []
            print_ec_mean = []; print_ec_std = []
            print_crash_mean = []; print_crash_std = []
            print_ndo_mean = []; print_ndo_std = []
            print_idu_mean = []; print_idu_std = []
            print_FX_mean = []; print_FX_std = []
            for i in range(ep // args.save_model_freq):
                s = args.save_model_freq
                print_reward_mean.append(round(np.mean(np.array(reward_log[i*s:(i+1)*s])),3))
                print_reward_std.append(round(np.std(np.array(reward_log[i*s:(i+1)*s])),3))
                print_ec_mean.append(round(np.mean(np.array(ec_log[i*s:(i+1)*s])),3))
                print_ec_std.append(round(np.std(np.array(ec_log[i*s:(i+1)*s])),3))
                print_crash_mean.append(round(np.mean(np.array(crash_log[i*s:(i+1)*s])),3))
                print_crash_std.append(round(np.std(np.array(crash_log[i*s:(i+1)*s])),3))
                print_ndo_mean.append(round(np.mean(np.array(ndo_log[i*s:(i+1)*s])),3))
                print_ndo_std.append(round(np.std(np.array(ndo_log[i*s:(i+1)*s])),3))
                print_idu_mean.append(round(np.mean(np.array(idu_log[i*s:(i+1)*s])),3))
                print_idu_std.append(round(np.std(np.array(idu_log[i*s:(i+1)*s])),3))
                print_FX_mean.append(round(np.mean(np.array(FX_log[i*s:(i+1)*s])),3))
                print_FX_std.append(round(np.std(np.array(FX_log[i*s:(i+1)*s])),3))
