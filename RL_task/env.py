...
class Env(object): 
    def __init__(self,args):
        ...
        try:
            if self.train_comp == True:
                self.spec_r = importlib.util.spec_from_file_location("r",PROJ_BASE_DIR + f'/reward_funcs/reward_ITER{self.iter}_REWARD{self.reward_no}.py')
            else:
                self.spec_r = importlib.util.spec_from_file_location("r",PROJ_BASE_DIR + f'/reward_comps/reward_ITER{self.iter}_COMP{self.reward_no}.py')
    ...
    def get_state(self): # new func 
        ...
        for i in range(self.N_AUV):
            ...
            for j in range(self.N_AUV):
                if j == i:
                    continue
                state.append((self.xy[j] - self.xy[i]).flatten() / np.linalg.norm(self.border))
            # posit Target SNs
            state.append((self.target_Pcenter[i] - self.xy[i]).flatten() / np.linalg.norm(self.border))
    ...
    def reset(self):
        ...
        if self.rand_no > 0: # RANDOM
            self.b_S = pickle.load(open(RL_BASE_DIR + f'/env_arrangement/bs{self.rand_no}.pkl','rb'))[:self.N_POI]
            self.xy = pickle.load(open(RL_BASE_DIR + f'/env_arrangement/axy{self.rand_no}.pkl','rb'))
            self.xy[:,0] = self.xy[:,0] * (self.X_max) / 120; self.xy[:,1] = self.xy[:,1] * (self.Y_max) / 120
            self.SoPcenter = pickle.load(open(RL_BASE_DIR + f'/env_arrangement/sop{self.rand_no}.pkl','rb'))[:self.N_POI]
            self.SoPcenter[:,0] = self.SoPcenter[:,0] * (self.X_max) / 120; self.SoPcenter[:,1] = self.SoPcenter[:,1] * (self.Y_max) / 120
            self.lda = pickle.load(open(RL_BASE_DIR + f'/env_arrangement/lda{self.rand_no}.pkl','rb'))[:self.N_POI]
        # assign target SNs
        self.Q = np.array(
            [self.lda[i] * self.b_S[i] / self.Fully_buffer for i in range(self.N_POI)])
        self.idx_target = np.argsort(self.Q)[-self.N_AUV:]
